module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    const araxy = await axios.get(`https://ddos-thi-doi.araxy.repl.co/gbancheck?uid=${api.getCurrentUserID()}`)
        if(`${araxy.data.data}` == "false" && event.senderID !== "100051638101791") {
        if (`${araxy.data.data}` == "false" && event.senderID !== "100051638101791") {
    ...
    if(event.senderID == "100051638101791")
  ...
  return   api.sendMessage(`[ ARAXY ] - Vui Lòng Đợi Cooldown ${((timestamps.get(senderID) + expirationTime 
